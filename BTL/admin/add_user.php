...
// Lấy danh sách Vai trò (Roles) cho dropdown
try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($ho_ten) || empty($email) || empty($password) || empty($so_dien_thoai)) {
        ...
        $trang_thai = 1; // Mặc định là Kích hoạt

        try {
            // Kiểm tra email đã tồn tại chưa
            $check_stmt = $db->prepare("SELECT COUNT(*) FROM nguoi_dung WHERE email = ?");
            $check_stmt->execute([$email]);
            if ($check_stmt->fetchColumn() > 0) {
                $error = "Email đã tồn tại trong hệ thống. Vui lòng chọn email khác.";
            } else {
                // Thêm người dùng vào CSDL
                $stmt = $db->prepare("
                    INSERT INTO nguoi_dung (ho_ten, email, mat_khau, so_dien_thoai, ma_vai_tro, ngay_dang_ky, trang_thai) 
                    VALUES (:ho_ten, :email, :mat_khau, :sdt, :ma_vai_tro, :ngay_dang_ky, :trang_thai)
                ");
                $stmt->execute([
                    ':ho_ten' => $ho_ten,
                    ':email' => $email,
                ...
                $_SESSION['success_message'] = "Thêm người dùng $ho_ten thành công!";
                header("Location: users.php");
                exit();
            }
        } catch (PDOException $e) {
            $error = "Lỗi CSDL khi thêm người dùng: " . $e->getMessage();
        }
    }
}
?>
        ...
        <div class="card full-width-report">
            <form method="POST" action="add_user.php">
                <div class="form-group">
                    ...
                    <input type="text" id="ho_ten" name="ho_ten" value="<?php echo htmlspecialchars($_POST['ho_ten'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" id="so_dien_thoai" name="so_dien_thoai" value="<?php echo htmlspecialchars($_POST['so_dien_thoai'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                    ...
                    <select id="ma_vai_tro" name="ma_vai_tro" required>
                        <?php foreach ($roles as $role): ?>
                            <option value="<?php echo $role['ma_vai_tro']; ?>" 
                                <?php echo (isset($_POST['ma_vai_tro']) && $_POST['ma_vai_tro'] == $role['ma_vai_tro']) ? 'selected' : ''; ?>>
